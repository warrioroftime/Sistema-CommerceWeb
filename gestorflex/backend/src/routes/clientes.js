...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (busca) {
      where += ` AND (nome ILIKE @b OR documento ILIKE @b OR email ILIKE @b)`;
      params.b = `%${busca}%`;
    }
    ...
    const data = r.recordset.map(c => ({
      ...c,
      qtd_compras: parseInt(statsMap[c.id]?.qtd_compras || 0),
      total_gasto: parseFloat(statsMap[c.id]?.total_gasto || 0),
    }));

    const tot = await query(`SELECT COUNT(*) AS n FROM Clientes WHERE ${where}`, params);
    res.json({ data, total: parseInt(tot.recordset[0].n) });
  } catch (err) {
    console.error(err);
  ...
  try {
    ...
    const vendas = await query(`
      SELECT v.id, v.criado_em, v.total, v.desconto, v.subtotal,
             ...
             (SELECT string_agg(p.descricao || ' (' || CAST(iv.quantidade AS TEXT) || ')', ', ')
